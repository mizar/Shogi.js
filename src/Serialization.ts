...
export function fromPreset(shogi: Shogi, setting: ISettingType): void {
  const board = [];
  const hands = [[], []];
  let turn;
...
export function toCSA(shogi: Shogi): string {
  const ret = [];
  for (let y = 0; y < 9; y++) {
    let line = "P" + (y + 1);
    for (let x = 8; x >= 0; x--) {
...
export function fromSfen(shogi: Shogi, sfen: string): void {
  const board = [];
  for (let i = 0; i < 9; i++) {
    board[i] = [];
    for (let j = 0; j < 9; j++) {
...
export function toSfen(shogi: Shogi, moveCount: number | string): string {
  const ret = [];
  const sfenBoard = [];
  for (let y = 0; y < 9; y++) {
    let line = "";
